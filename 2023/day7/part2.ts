...
class Hand {
  ...
  constructor(public cards: Card[]) {
    ...
    const jokersPos = this.qNotation.findIndex((card) => card[0] === "J");

    if (jokersPos === 0) {
      if (this.qNotation.length === 1) {
        this.qNotation[0][0] = "A";
      } else {
        this.qNotation[1][1] += this.qNotation[0][1];
        this.qNotation.shift();
      }
    } else if (jokersPos > 0) {
      this.qNotation[0][1] += this.qNotation[jokersPos][1];
      this.qNotation.splice(jokersPos, 1);
    }
  }
}
